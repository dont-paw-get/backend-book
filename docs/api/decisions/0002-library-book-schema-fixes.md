...
5. `UpdateLibraryBookRequest`로 `bookNumber`·`language`를 수정할 수 있지만 `UpdateLibraryBookResponse`는 그 값을 돌려주지 않아, PATCH 이후 실제로 반영된 값을 응답만으로 확인할 수 없었다.
6. `coverUrl`은 등록 요청(`CreateLibraryBookRequest`)에만 있고 `UpdateLibraryBookRequest`에는 없어, 등록 이후 표지를 다시 지정할 방법이 없었다.
...
5. `bookNumber`·`language`를 `UpdateLibraryBookResponse`에 필수 필드로 추가해 PATCH 결과를 응답에서 확인할 수 있게 한다.
6. `coverUrl`을 `UpdateLibraryBookRequest`, `UpdateLibraryBookResponse`에 선택 필드로 추가해 등록 후에도 표지를 변경할 수 있게 한다.

## 결과
- 서재 도서의 언어·표지·서재 내 순번이 등록부터 조회·수정까지 계약상 일관되게 노출된다.
- 값을 채울 방법이 없던 `coverColors` 필터가 계약에서 제거돼 클라이언트가 오해할 여지가 줄어든다.
- PATCH 요청으로 바뀐 값을 응답으로 즉시 확인할 수 있어 클라이언트가 재조회 없이 최신 상태를 반영할 수 있다.
