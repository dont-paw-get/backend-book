...
- `k8s/overlays/prod/configmap-patch.yaml`의 `AUTH_ISSUER_URI`/`AUTH_APP_CLIENT_ID`가 dev overlay와 동일한 User Pool(`ap-northeast-2_y1mKz50El`)을 가리킨다 — 2026-08-27 prod CD 파이프라인 검증을 위해 의도적으로 공용한 것이므로, 상용 전용 User Pool이 준비되면 prod overlay만 그 값으로 교체한다. 이때 **backend-auth prod overlay에도 `COGNITO_BACKEND_CLIENT_ID`가 없다(placeholder `COGNITO_CLIENT_ID`만 있음)** — 두 저장소의 prod App Client 설정을 같은 작업에서 맞춰야 한다(CLIAR-188).
...
- `k8s/overlays/{dev,prod}/configmap-patch.yaml`의 `OTEL_EXPORTER_OTLP_ENDPOINT`가 아직 관례 기본값(`http://opentelemetry-collector.observability.svc.cluster.local:4318`)이다(CLIAR-200) — 인프라 저장소에서 실제 Collector Service 이름/네임스페이스가 확정되면 양쪽 overlay를 그 값으로 교체한다. 닿지 못해도 앱은 정상 동작하고 exporter 경고만 남는다.
- 메트릭은 이번 범위 밖이라 껐다(`management.otlp.metrics.export.enabled: false`, CLIAR-200) — `micrometer-registry-otlp`는 이미 클래스패스에 있으므로, Collector에 metrics 파이프라인이 준비되면 이 값만 켜면 된다.
- OTLP **로그** 익스포트(`management.logging.export.otlp.enabled`)도 쓰지 않는다 — 로그 경로는 stdout → Alloy → Loki 하나로 유지하기로 했다(CLIAR-200). Alloy 없이 앱이 직접 Loki/Collector로 로그를 보내는 구조가 필요해지면 재검토.
- 다른 MSA(auth·record)와의 **end-to-end 단일 trace 연결은 아직 실물로 확인하지 못했다**(CLIAR-200) — Book Service 쪽은 W3C `traceparent` 송수신이 기본 활성이지만, 상대 서비스가 같은 규격으로 계측되어 있어야 한 trace로 이어진다. Collector가 뜨면 실제 호출로 확인한다.
