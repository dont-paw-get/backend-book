...
- 도서 검색은 두 경로가 있다:
  - `GET /api/v1/books/search` — **isbn 기준**(ADR-0012). 사용자가 바코드 스캔 등으로 이미 확보한 isbn을 넘긴다. 서재 등록 여부(`alreadyRegistered`)를 함께 확인한다.
  - `GET /api/v1/books/search/by-title-author` — **제목·저자 기준**(ADR-0013). isbn을 모를 때 쓴다. 서버가 알라딘 `ItemSearch`를 제목으로 한 번, 저자로 한 번 호출해 두 결과의 교집합(isbn 동일)을 구하고, 제목 검색 결과 순서 기준 최상단 1권만 반환한다. 교집합이 비면 `book`이 생략된다(사용자 직접 입력 폴백). 이 경로는 서재 등록 여부를 확인하지 않는다.
