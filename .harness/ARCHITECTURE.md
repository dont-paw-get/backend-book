...
├─ librarian                             # library와 동일한 4계층(domain/application/infrastructure/web) 구조. 회원별 대표 사서 선택은 Member 서비스로 이관됐고(ADR-0009) 사서 마스터 카탈로그 조회만 남았다
│  ├─ domain
...
│  │  └─ LibrarianService.java           # getLibrarians
│  ├─ infrastructure
│  │  └─ LibrarianJpaRepository.java / LibrarianRepositoryJpaAdapter.java
│  └─ web
│     ├─ LibrarianController.java        # GET /api/v1/librarians
│     └─ dto                             # LibrarianListResponse/LibrarianSummary — openapi 스키마 1:1
└─ security
...
└─ db/migration
   ...
   └─ V6__drop_member_librarian_selection.sql  # 대표 사서 선택이 Member 서비스로 이관되어 member_librarian_selection 테이블 DROP(ADR-0009). librarian(마스터)은 유지
...
├─ librarian/application/LibrarianServiceTest.java      # Mockito — 카탈로그 조회
