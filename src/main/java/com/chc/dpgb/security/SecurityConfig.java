...
public class SecurityConfig {
	...
	public SecurityFilterChain securityFilterChain(
			HttpSecurity http, @Lazy JwtDecoder jwtDecoder, AuthenticationEntryPoint authenticationEntryPoint)
			throws Exception {
		http
	...
	/**
	 * issuer-uri는 실제 Cognito User Pool이 없으면 앱 기동 시점에 discovery
	 * 네트워크 호출이 실패한다. 빈 생성을 지연시켜, 인증이 필요한 요청이 실제로
	 * 들어오기 전까지는(User Pool 준비 전까지는) 컨텍스트 기동이 막히지 않게 한다.
	 */
	@Bean
	@Lazy
